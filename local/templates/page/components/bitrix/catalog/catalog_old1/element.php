...
//	echo '<pre>'; var_dump($_POST); echo '</pre>';
	if (isset($_POST["lot"]) && isset($_POST["cost"]) && isset($_POST["costMin"])){
		if ($_POST["cost"] >= $_POST["costMin"]){
...
//    	изменить лот
//			echo "Сохранено";
			$res = CIBlockElement::GetProperty(1, $ibElemId, "sort", "asc", array("CODE" => "CNT_ST"));
			if ($ob = $res->GetNext())
			{
				$cntST = (empty($ob['VALUE'])? 1:$ob['VALUE']+1);
			}
//			 Установим новое значение для данного свойства данного элемента
			CIBlockElement::SetPropertyValuesEx($ibElemId, false, array("CNT_ST" => $cntST ,"PRICE_CUR" => $newCost));

			BXClearCache(true);

//			$ell = new CIBlockElement;
//			$PROP = array("CNT_ST" => $cntST ,"PRICE_CUR" => $newCost);
//
//			$arLArray = Array(
...
//			);
//			$resl = $ell->Update($ibElemId, $arLArray);
		} else {
			echo 'Ошибка';
		}
		}
